var calculateScore = (s) => {
  ...
  for (let i = 0; i < 26; i++) {
    mirrorMap[alphabet[i]] = alphabet[25 - i];
  }
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (unmarkedIndices[mirrorChar]?.length > 0) {
      const closestIndex = unmarkedIndices[mirrorChar].pop();
      score += i - closestIndex;
    } else {
      if (!unmarkedIndices[currentChar]) {
        unmarkedIndices[currentChar] = [];
      }
      unmarkedIndices[currentChar].push(i);
    }
  }
